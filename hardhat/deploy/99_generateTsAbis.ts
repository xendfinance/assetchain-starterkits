...
function getContractDataFromDeployments() {
  ...
  for (const chainName of getDirectories(DEPLOYMENTS_DIR)) {
    ...
    let contracts = {} as Record<string, any>;
    for (const contractName of getContractNames(`${DEPLOYMENTS_DIR}/${chainName}`)) {
      ...
      contracts = { contractName, chainId, address, abi, inheritedFunctions };
    }
...
const generateTsAbis: DeployFunction = async function () {
  const TARGET_DIR = "../frontend/src/contracts/";
  ...
  console.log(`📝 Updated TypeScript contract definition file on: ${TARGET_DIR}`);
};
